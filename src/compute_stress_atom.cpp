  ...
  id_temp(NULL), stress(NULL)
{
  ...
  if (narg == 4) {
    ...
    while (iarg < narg) {
      ...
      else if (strcmp(arg[iarg],"intrapair") == 0) intrapairflag = 1; /* sasaki  2019/02/23 */
      else if (strcmp(arg[iarg],"interpair") == 0) interpairflag = 1; /* sasaki  2019/02/23 */
...
void ComputeStressAtom::compute_peratom()
{
  ...
  if (pairflag && force->pair) {
    ...
    for (i = 0; i < npair; i++)
      for (j = 0; j < 6; j++)    /* j is  xx yy zz xy yz zx */
        stress[i][j] += vatom[i][j];
  }

/* sasaki  2019/02/23 03/01 */
  if (intrapairflag && force->pair) {
    double **vatom = force->pair->vatomra;
      for (i = 0; i < npair; i++)
        for (j = 0; j < 6; j++)
          stress[i][j] += vatom[i][j];
  }
/* sasaki  2019/02/23 */
  if (interpairflag && force->pair) {
    double **vatom = force->pair->vatomer;
    for (i = 0; i < npair; i++)
      for (j = 0; j < 6; j++)
        stress[i][j] += vatom[i][j];
  }
