...
Pair::Pair(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  maxeatom = maxvatom = maxmolid = maxvatomer = 0;/* sasaki  2019/03/28 */
  eatom = NULL;
  vatom = NULL;
  vatomer = NULL;
  vatomra = NULL;
  molid = NULL;/* sasaki  2019/03/28 */
...
Pair::~Pair()
{
  ...
  memory->destroy(vatomer);
  memory->destroy(vatomra);
}
...
void Pair::ev_setup(int eflag, int vflag, int alloc)
{
  ...
  /*////////////// sasaki 2019/03/28 //////////////*/
  if (vflag_atom && atom->nmax > maxmolid) {
    maxmolid = atom->nmax;
    if (alloc) {
      memory->destroy(molid);
      memory->create(molid,comm->nthreads*maxmolid,"pair:maxmolid");
    }
  }

  if (vflag_atom && atom->nmax > maxvatomer) {
    maxvatomer = atom->nmax;
    if (alloc) {
      memory->destroy(vatomer);
      memory->destroy(vatomra);
      memory->create(vatomer,comm->nthreads*maxvatomer,6,"pair:vatomer");
      memory->create(vatomra,comm->nthreads*maxvatomer,6,"pair:vatomra");
    }
  }
  /*////////////// sasaki 2019/03/28 //////////////*/
  ...
  if (vflag_atom && alloc) {
    ...
    if (force->newton) n += atom->nghost;
    for (i = 0; i < n; i++) {
      molid[i]    = atom->molecule[i];//sasaki  2019/03/27
      vatom[i][0] = 0.0;
      vatom[i][1] = 0.0;
      vatom[i][2] = 0.0;
      vatom[i][3] = 0.0;
...
double Pair::memory_usage()
{
  ...
  bytes += comm->nthreads*maxvatom*6 * sizeof(double);//inter sasaki
  bytes += comm->nthreads*maxvatom*6 * sizeof(double);//intra sasaki
  bytes += comm->nthreads*maxmolid * sizeof(double);//molid sasaki
  return bytes;
}
